...
public:
    interp(double *vpts,   double* vspacing, double* vsup, double* vnumComp, double* vszParam, double* vnumelP, double* vlevels, double* vpLevelS, double* vvoxelSize, double* vdo_derivative, double* vnewPts, double* vdDP, int* vidxx, double* vdiff1, double* vdiff2, double* vdiff3, int vN_pts, const mxArray *vdata, int vsupportLast)
    {
    ...
    for(int i=r.begin();i!=r.end();i++)
    {
        ...
        for (int c=0; c<(int)numComp[0];c++)
        {
                ...
                for (int j=0; j<supportCurrent;j++)
                {
                    for (int k=0; k<supportCurrent;k++)
                    {
                            ...
                            
                        {
                            ...
                            {
                                //current dfdp
                                dfdp[i_idx_idx_1][0]=dd;
                                dfdp[i_idx_idx_1+supportCurrent*supportCurrent*supportCurrent][0]=0;
                                dfdp[i_idx_idx_1+supportCurrent*supportCurrent*supportCurrent*2][0]=0;
                                
                                dfdp[i_idx_idx_1][1]=0;
                                dfdp[i_idx_idx_1+supportCurrent*supportCurrent*supportCurrent][1]=dd;
                                dfdp[i_idx_idx_1+supportCurrent*supportCurrent*supportCurrent*2][1]=0;
                                
                                dfdp[i_idx_idx_1][2]=0;
                                dfdp[i_idx_idx_1+supportCurrent*supportCurrent*supportCurrent][2]=0;
                                dfdp[i_idx_idx_1+supportCurrent*supportCurrent*supportCurrent*2][2]=dd;
                                
                                
                                if(c==0)
                                {
                                    
                                    idxx[i_idx_idx_2+i*supportCurrent*supportCurrent*supportCurrent]=linearindex;
                                    i_idx_idx_2++;
            ...
            for(int rr=0;rr<supportLast*supportLast*supportLast*3;rr++)
            {
                for(int cc=0;cc<3;cc++)
                {
                    ...
                    {
                        temp[rr][cc]=temp[rr][cc]+dDP[rr+kk*(supportLast*supportLast*supportLast*3)+i*(supportLast*supportLast*supportLast*3*3)]*jacNew[kk][cc];
                    }
                }
            }
            
            
            for (int ii=0; ii<supportLast*supportLast*supportLast*3; ii++)
            {
                
                dDP[ii+0*(supportLast*supportLast*supportLast*3)+i*(supportLast*supportLast*supportLast*3*3)]=temp[ii][0]+dfdp[ii][0];
                dDP[ii+1*(supportLast*supportLast*supportLast*3)+i*(supportLast*supportLast*supportLast*3*3)]=temp[ii][1]+dfdp[ii][1];
                dDP[ii+2*(supportLast*supportLast*supportLast*3)+i*(supportLast*supportLast*supportLast*3*3)]=temp[ii][2]+dfdp[ii][2];
            }
...
//support of the level needed
    ...
    // output dfdp and delete val
    mwSize dims_d[3];
    dims_d[0]=supportLast*supportLast*supportLast*3;
    dims_d[1]=3;
    dims_d[2]=N_pts;
    
    plhs[2] = mxCreateNumericArray(3,dims_d,mxDOUBLE_CLASS, mxREAL);
    double* dDP = static_cast<double*>(mxGetData(plhs[2]));
    
    for (int i=0;i<supportLast*supportLast*supportLast*3*N_pts*3;i++)
    {
        dDP[i]=0;
    }
    ...
    mwSize dims_idx[2];
    dims_idx[0]=supportLast*supportLast*supportLast;
    dims_idx[1]=N_pts;
    
    plhs[1] = mxCreateNumericArray(2,dims_idx,mxINT32_CLASS, mxREAL);;
    int* idxx = static_cast<int*>(mxGetData(plhs[1]));
    ...
    double* diff1 = static_cast<double*>(mxGetData(plhs[3]));
    
    plhs[4] = mxCreateNumericArray(2,dims_jac,mxDOUBLE_CLASS, mxREAL);
    double* diff2 = static_cast<double*>(mxGetData(plhs[4]));
    
    plhs[5] = mxCreateNumericArray(2,dims_jac,mxDOUBLE_CLASS, mxREAL);
    double* diff3 = static_cast<double*>(mxGetData(plhs[5]));
    


    parallel_for(blocked_range<int>(0,N_pts),interp(pts, spacing, sup, numComp, szParam, numelP, levels, pLevelS, voxelSize, do_derivative, newPts, dDP, idxx, diff1, diff2, diff3, N_pts, data, supportLast),auto_partitioner());
